    ...
    public class PpPlus : AppConstruct
    {
        ...
        private static Bitmap Draw(string user, IReadOnlyDictionary<string, int> dPfmance)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                // 画数据边框
                Color cBderStatic = Color.FromArgb(90 + (int)((241 - 90) * dPfmance["Performance"] / maxPp),
                                            180 + (int)((139 - 180) * dPfmance["Performance"] / maxPp),
                                             49 + (int)((37 - 49) * dPfmance["Performance"] / maxPp));

                using (Brush brush = new SolidBrush(cBderStatic))
                using (Pen pen = new Pen(cBderStatic, 2))
                {
                    for (int i = 0; i < pointStatic.Length; i++)
                    {
                        g.FillEllipse(brush, new RectangleF(pointStatic[i].X - 3, pointStatic[i].Y - 3, 6, 6));

                        if (i == pointStatic.Length - 1)
                            g.DrawLine(pen, pointStatic[i], pointStatic[0]);
                        else
                            g.DrawLine(pen, pointStatic[i], pointStatic[i + 1]);
                ...
                using (Brush blackBrush = new SolidBrush(Color.Black))
                {
                    foreach (var item in dPfmance)
                    {
                        ...
                        float deg = (float)(ii * 60d / 180d * Math.PI);
                        var tmpPoint = new PointF((float)(pointCentre.X + Math.Sin(deg) * rCurrent) - ok.Width / 2f, pointCentre.Y + (float)(Math.Cos(deg) * rCurrent) - ok.Height / 2f);

                        g.DrawString(item.Value.ToString(), fontMsyh, blackBrush, tmpPoint);
                ...
                using (Brush brushTotal = new SolidBrush(Color.FromArgb(70, cBderStatic.R, cBderStatic.G, cBderStatic.B)))
